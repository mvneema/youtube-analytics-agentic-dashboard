...
Use this skill when the user invokes `/fable-project` to turn a rough idea into a clarified plan and, when appropriate, an implementation.
...
- `/fable-project` — start from scratch and interview the user step by step.
- `/fable-project <rough task>` — use the task as the initial known known and begin the workflow.
- `/fable-project discover` — run intake, blind-spot pass, unknowns board, and interview only.
- `/fable-project brainstorm` — run intake, blind-spot pass, and solution brainstorming.
- `/fable-project prototype` — run intake, blind-spot pass, and create quick prototypes or mockups before implementation.
- `/fable-project plan` — produce or refine an implementation plan from the current context.
- `/fable-project implement` — implement an approved plan, or ask for the missing plan/approval.
- `/fable-project review` — produce an explainer, risk review, and quiz after implementation.

If `$ARGUMENTS` is empty, vague, or simply `prompt`, start with the Discovery Interview. Treat `prompt` as an alias for the default `/fable-project` workflow, not as a separate command name.
...
For analytics/metrics tasks (e.g. YouTube, LinkedIn, podcast, or product dashboards), "inspect" means:

1. Check how each headline metric is defined and whether that definition could mislead (e.g. views vs. unique viewers, watch time vs. average view duration).
2. Check the time window: rolling vs. calendar period, and whether that framing distorts a comparison.
3. Check for outliers: is one item (video, post, day) carrying the aggregate?
4. Check for confounders external to content quality: cross-posts, paid pushes, platform/algorithm changes, seasonality.
5. Check what baseline the numbers are implicitly being compared against, and whether that baseline is valid.
6. Check what decision the data is meant to inform, and whether the current cut of data actually supports it.

Pull specific prompts from `reference/question-bank.md` → "Analytics / data blind-spot prompts" when running this pass.
